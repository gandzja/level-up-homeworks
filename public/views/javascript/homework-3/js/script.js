...
// действие выполнится так как в условии строка с цифрой 0, а з начит значение true.
...
// выведет 2 потомому что это значение true.
...
// вывдет значение null так как это первое значение false.
...
if (null || -1 && 1) console.log( 'третье' ); // выведет строку. Вначале выполняется && оба аоперанда true вернется начение 1, затем || ищет true и это снова 1.
...
'' - 1 + 0 // пустая строка это 0, минус оставляет еденицу цифрой и плюс ноль равно -1 
true + false // true - это 1, false - это 0. итого 1.
6 / '3' // . равно 2, так как опертор деления работает только с цифрами, то '3'преобразуется в число 3
...
'  -9\n' + 5 // число 5 преобразуется в '5', строки складіваются в '  -9\n5'
'  -9\n' - 5 // '  -9\n' - это '-9', преобразуется в число -9, дальше вычитается -5, итого -14
5 && 2 // 2 оператор И ищет false, и если его нет, то выводит последнийtrue
2 && 5 // 5 оператор И ищет false, и если его нет, то выводит последнийtrue
5 || 0 // 5 true, оператор ИЛИ ичет true и выводит его или воследниий false
0 || 5 // 0 false, 5 true, оператор ИЛИ ичет true и выводит его или воследниий false
...
+null == +'\n0\n' // +null - преобразуется в 0, +'\n0\n' - преобразуется в 0, они раны поєтому true.
